      ...
      >
        <template #empty>
          ...
          <div class="text-center">
            特别鸣谢:vue3 + vite + element-plus
            <el-button
              link
              type="primary"
              @click="openUrl('https://www.bilibili.com/video/BV1TR4y137ZX')"
            >
              使用说明</el-button
            >
            <el-button link type="primary" @click="openUrl('https://space.bilibili.com/329962610')">
              关于我</el-button
            >
          </div>
        </template>
        ...
        <el-table-column label="类型" width="120px" prop="filetype" align="center">
        </el-table-column>

        <el-table-column label="操作" width="180px">
          <template #default="{ row, $index }">
            <el-button link type="primary" @click="openPath(row.path)"> 📂 </el-button>
            <el-button link type="primary" @click="moveUp($index)"> ⬆️ </el-button>
            <el-button link type="primary" @click="moveDown($index)"> ⬇️ </el-button>

            <el-popconfirm @confirm="deleteFile($index)" title="确定要删除此项吗?">
              <template #reference>
                <el-button link type="primary">🗑️</el-button>
              </template>
            </el-popconfirm>
          </template>
        </el-table-column>
        <el-table-column prop="status" align="center" width="200px" label="状态">
          <template #default="{ row }">
            <el-button v-if="row.status == 2" link type="primary" @click="openPath(row.outpath)">
              <span>转换完成</span>
              ✅</el-button
            >
            <span v-else type="primary" :loading="row.status == 1">{{ filText(row.status) }}</span>
          </template>
        </el-table-column>
      </el-table>
    </div>
